...
public sealed class GPSEncoder {
    ...
    public static float CurrentDistance(Vector2 gps)
    {
        return GetInstance().haversine(gps.x, gps.y, GetInstance()._LatOrigin, GetInstance()._LonOrigin);
    }
    public static float CurrentDistance(float lat, float lon)
    {
        return GetInstance().haversine(lat, lon, GetInstance()._LatOrigin, GetInstance()._LonOrigin);
    }
